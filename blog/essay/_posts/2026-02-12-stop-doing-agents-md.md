---
layout: post
title: "Stop Doing AGENTS.md (this includes CLAUDE.md)"
author: texarkanine
tags:
  - ai
  - claude-code
...
[AGENTS.md](https://agents.md/) and its ilk - including `CLAUDE.md` are one of several kinds of AI agent customization techniques. We'll borrow from [a16n's taxonomy](https://texarkanine.github.io/a16n/models/#globalprompt) refer to them as a form of **GlobalPrompt**:
...
These are basic usage patterns of these tools; a modern agent should already know how these tools work and use them correctly. And if not, the CLIs' own `help` text will reveal - there is no need to duplicate this usage here.

This *would* have been handy for a human who showed up to the project maybe never having seen `pnpm` before, and who would therefore not recognize what was going on. 
...
And so on, and so forth. Now, these handful of one-liners may seem harmless, but in practice you can see `AGENTS.md` files baloon in size to hundreds of lines with paragraphs upon paragraphs of task-specific guidance... and the more of that you add, the smaller the percent of `AGENTS.md` that actually applies to the task at hand.
...
We have a tool for that that doesn't cost tokens: [pre-commit hooks](https://pre-commit.com/). It's way cheaper to use that than ask an LLM to 
...
And maybe that particular sentence is harmless. But [neither of those scales up well](https://research.trychroma.com/context-rot) as `AGENTS.md` gains section after section after [paragraph after paragraph of content that is *not* related to your specific task](https://github.com/github/spec-kit/blob/0049b1cdc2f9ba12def39a042872b0b1b6a09704/AGENTS.md).
...
Why should you even have to tell an agent to look into `pacakge.json` for how to invoke `eslint`, though? Is it [not `npx eslint` per the eslint docs](https://eslint.org/docs/latest/use/getting-started) or just `npm run lint`?

***Why not?***

Noticing what the agent gets wrong is the right first step. Paving your [desire paths](https://en.wikipedia.org/wiki/Desire_path) is better than buliding an ever-growing list of corrective prescriptions.
...
Hey, that's an [AgentSkill](https://agentskills.io) - a big set of information that is *not* automatically in context, hidden behind a little description indicating when it might be useful and/or when it should be pulled into context.
...
There *is* still a place for a **GlobalPrompt** though: use `AGENTS.md` as if you were building a sub-agent, but to bootstrap the core persona of your primary agent. Something like [niko-core.mdc](https://github.com/Texarkanine/.cursor-rules/blob/main/rules/niko-core.mdc). No specifics about the repo at all - just general guidelines for how the Agent should *be*.
