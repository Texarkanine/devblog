...
{% polaroid
	stockroom-dashboard-top-light.png
	link="https://texarkanine.github.io/stockroom/user-guide/dashboard/"
	title="Stockroom dashboard"
	alt="Stockroom local dashboard showing session history across harnesses"
	size=600x
%}

It backfills. Harness formats come and go; Stockroom reads what is still on disk, including formats the vendors already abandoned. On this machine the warehouse holds a bit over 113,000 messages across Cursor and Claude Code, reaching back to September 2025 - months before Stockroom's first commit on 2026-06-22. The cameras were running. Stockroom just lets you re-watch the tapes.

That "pays backwards" feeling is what [Just Try the Thing]({% post_url blog/essay/2026-08-01-just-try-the-thing %}) was pointing at: agents can use work from *before* the warehouse existed, because the raw logs were already on disk. [Disintegration of Persistence of Memory.md]({% post_url blog/essay/2026-08-03-disintegration-of-persistence-of-memory-md %}) puts the same store on the memory ladder as rung five - security-camera footage for *how* and *why*, not a notepad you hope somebody kept tidy. Those posts argue the need. This one is the install.
