...
Everything MCP puts into a model's context window has a cost: tokens for the tool definitions, tokens for the schemas, attention spent considering tools that may not be relevant to the task at hand. If you've read "[Stop Doing AGENTS.md](/2026/02/12/stop-doing-agents-md.html)", you already know where I stand on paying context rent for things that aren't pulling their weight. MCP is no different. A tool definition sitting in context is overhead until the moment it's invoked - and if the model *might not* invoke it, you're speculating with your context budget.
